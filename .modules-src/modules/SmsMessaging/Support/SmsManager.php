...
class SmsManager
{
    ...
    public function send(string $to, string $body, ?Model $notifiable = null): SmsMessage
    {
        $normalised = PhoneNumber::normalise($to);

        if ($normalised === null) {
            return $this->record($to, $body, SmsMessage::STATUS_FAILED, error: 'Unrecognised phone number.', notifiable: $notifiable);
        }

        if (SmsOptOut::has($normalised)) {
            return $this->record($normalised, $body, SmsMessage::STATUS_SUPPRESSED, error: 'Recipient has opted out.', notifiable: $notifiable);
        }

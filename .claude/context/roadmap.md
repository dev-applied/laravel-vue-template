...
- [ ] `module:add`'s finish-up note says to restart the vite dev server, which is not enough: the Vuetify plugin's virtual modules stay cached and every component 404s on its .sass until `node_modules/.vite` is removed. Hit three times tonight. Either document `rm -rf node_modules/.vite` in the note or have the command clear it.

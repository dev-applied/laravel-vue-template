...
- [ ] **Unknown Vuetify utility classes are silent too** — a typo in `justify-space-between` or `text-medium-emphasis` contributes nothing and reports nothing. The four class-shaped guards above each name one known-bad pattern; the general form is to check every static class token against the set Vuetify actually ships plus the project's own SCSS, and report utility-shaped tokens that exist in neither.
